...
def get_rankings(year, region, gender, age_group, event):
    ...
    Returns a list of ranks for given year, region, gender, age group and event.
            ...
            Returns:
                    - 'rankings' (arr): list of ranks
                        ...
                        - 'performance' (str): Performance of athlete for given rank
                        - 'pb' (str): Personal best of athlete at time of rank
    ...
    if None in (year, region, gender, age_group, event):
        raise ValueError('Please ensure all search fields are filled.')

    url = f'https://www.thepowerof10.info/rankings/rankinglist.aspx?event={event}&agegroup={age_group}&sex={gender}&year={year}'
    ...
    if region is not None:
        try:
            url += f'&areaid={locs[region]}'
        except Exception as e:
            raise ValueError('Please ensure the location you have provided is valid.')
